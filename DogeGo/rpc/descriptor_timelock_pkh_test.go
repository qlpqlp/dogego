// Copyright (c) 2026 Paulo Vidal (https://x.com/inevitable360, https://github.com/qlpqlp)
// Copyright (c) 2026 Dogecoin Foundation
//
// SPDX-License-Identifier: MIT
// See LICENSE for copyright attribution to upstream Bitcoin/Dogecoin Core.

package rpc

import (
	"encoding/hex"
	"encoding/json"
	"path/filepath"
	"testing"

	"dogego/secp256k1"

	"dogego/chain"
	"dogego/consensus"
	"dogego/store"
	"dogego/wallet"
	"dogego/wire"
)

func TestImportDescriptorsShCLTVPKH(t *testing.T) {
	p, _ := chain.ParamsFor(chain.RebootTestnet)
	k, _ := secp256k1.NewPrivateKey()
	wif, _ := chain.EncodeWIF(k.Serialize(), p.PrivKeyWIFVersion, true)
	addr, err := addressFromWIF("test", wif)
	if err != nil {
		t.Fatal(err)
	}
	desc := "sh(cltv(100)pkh(" + addr + "))"
	w, err := wallet.LoadOrCreate(filepath.Join(t.TempDir(), "wallet.json"), p.PubkeyHashAddrID)
	if err != nil {
		t.Fatal(err)
	}
	paths := importDescTestPaths(w, p)
	paths.Utxo = store.NewUtxoCache()
	req, _ := json.Marshal(map[string]interface{}{"desc": desc, "keys": []string{wif}})
	_, code, msg := execImportDescriptors("test", paths, nil, nil, []json.RawMessage{json.RawMessage("[" + string(req) + "]")})
	if code != 0 {
		t.Fatalf("importdescriptors: %s", msg)
	}
	redeem := w.WatchRedeemScript(w.WatchScripts()[0])
	lock, ok := consensus.CLTVLockTimeFromRedeem(redeem)
	if !ok || lock != 100 {
		t.Fatalf("lock=%d ok=%v", lock, ok)
	}
}

func TestSignRawTransactionWithWalletImportDescriptorsShCLTVPKH(t *testing.T) {
	p, _ := chain.ParamsFor(chain.RebootTestnet)
	k, _ := secp256k1.NewPrivateKey()
	wif, _ := chain.EncodeWIF(k.Serialize(), p.PrivKeyWIFVersion, true)
	addr, err := addressFromWIF("test", wif)
	if err != nil {
		t.Fatal(err)
	}
	desc := "sh(cltv(100)pkh(" + addr + "))"
	w, err := wallet.LoadOrCreate(filepath.Join(t.TempDir(), "wallet.json"), p.PubkeyHashAddrID)
	if err != nil {
		t.Fatal(err)
	}
	paths := importDescTestPaths(w, p)
	paths.Utxo = store.NewUtxoCache()
	req, _ := json.Marshal(map[string]interface{}{"desc": desc, "keys": []string{wif}})
	_, code, msg := execImportDescriptors("test", paths, nil, nil, []json.RawMessage{json.RawMessage("[" + string(req) + "]")})
	if code != 0 {
		t.Fatalf("importdescriptors: %s", msg)
	}
	p2sh := w.WatchScripts()[0]
	fundTx := &wire.Tx{
		Version: 1,
		Vin:     []wire.TxIn{{Sequence: 0xffffffff}},
		Vout:    []wire.TxOut{{Value: 2e8, PkScript: p2sh}},
	}
	_ = paths.Utxo.ApplyBlock(&wire.ParsedBlock{Txs: []*wire.Tx{fundTx}}, 0)
	prev := fundTx.TxHash()
	payTo, _ := chain.RandomP2PKHAddress(p)
	_, h160, _ := chain.Base58CheckDecode(payTo)
	spend := &wire.Tx{
		Version: 1,
		Vin:     []wire.TxIn{{PrevHash: prev, PrevIdx: 0, Sequence: wire.MaxBIP125RBFSequence}},
		Vout:    []wire.TxOut{{Value: 1e8, PkScript: chain.P2PKHScriptFromPubKeyHash(h160)}},
	}
	spendHex, _ := spend.Serialize()
	res, code2, msg2 := execSignRawTransactionWithWallet("test", paths, []json.RawMessage{
		json.RawMessage(`"` + hex.EncodeToString(spendHex) + `"`),
	})
	if code2 != 0 {
		t.Fatalf("sign: %d %s", code2, msg2)
	}
	if !res["complete"].(bool) {
		t.Fatalf("errors=%v", res["errors"])
	}
}
