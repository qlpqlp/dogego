...
func TestFundRawTransactionAutoChangeAddress(t *testing.T) {
	...
	_, h160, _ := chain.Base58CheckDecode(w.DefaultAddress())
	...
	paths := &DataPaths{
		...
		WalletNewChangeAddress: func() (string, error) { return changeAddr, nil },
	}
	...
	if m["changepos"].(int) < 0 {
		t.Fatalf("changepos %#v", m["changepos"])
	}
}
