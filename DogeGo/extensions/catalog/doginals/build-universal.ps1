...
$platforms = @(
    @{ Key = "windows-amd64"; GOOS = "windows"; GOARCH = "amd64"; Out = "doginals-ext.exe" },
    @{ Key = "windows-arm64"; GOOS = "windows"; GOARCH = "arm64"; Out = "doginals-ext.exe" },
    @{ Key = "linux-amd64";   GOOS = "linux";   GOARCH = "amd64"; Out = "doginals-ext" },
    @{ Key = "linux-arm64";   GOOS = "linux";   GOARCH = "arm64"; Out = "doginals-ext" },
    @{ Key = "darwin-amd64";  GOOS = "darwin";  GOARCH = "amd64"; Out = "doginals-ext" },
    @{ Key = "darwin-arm64";  GOOS = "darwin";  GOARCH = "arm64"; Out = "doginals-ext" }
)
...
foreach ($p in $platforms) {
    ...
    $env:GOARCH = $p.GOARCH
    $env:CGO_ENABLED = "0"
    go build -ldflags="-s -w" -trimpath -o $out ./cmd/doginals-ext
...
$zip = Join-Path $dist "doginals-universal.zip"
if (Test-Path $zip) { Remove-Item $zip -Force }
Compress-Archive -Path (Join-Path $stage "*") -DestinationPath $zip
Remove-Item $manifestPath -Force
Remove-Item $stage -Recurse -Force

$legacy = Join-Path $dist "doginals.zip"
