...
$platforms = @(
    @{ Key = "windows-amd64"; GOOS = "windows"; GOARCH = "amd64"; Out = "radiodoge-ext.exe" },
    @{ Key = "windows-arm64"; GOOS = "windows"; GOARCH = "arm64"; Out = "radiodoge-ext.exe" },
    @{ Key = "linux-amd64";   GOOS = "linux";   GOARCH = "amd64"; Out = "radiodoge-ext" },
    @{ Key = "linux-arm64";   GOOS = "linux";   GOARCH = "arm64"; Out = "radiodoge-ext" },
    @{ Key = "darwin-amd64";  GOOS = "darwin";  GOARCH = "amd64"; Out = "radiodoge-ext" },
    @{ Key = "darwin-arm64";  GOOS = "darwin";  GOARCH = "arm64"; Out = "radiodoge-ext" }
)
...
$zip = Join-Path $dist "radiodoge-universal.zip"
if (Test-Path $zip) { Remove-Item $zip -Force }
Compress-Archive -Path (Join-Path $stage "*") -DestinationPath $zip
Copy-Item $zip (Join-Path $dist "radiodoge.zip") -Force
