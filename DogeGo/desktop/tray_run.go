...
func platformTrayRequiresMainThread() bool {
	// AppKit / NSStatusItem must run on the process main thread; a locked background
	// OS thread still SIGTRAPs on Darwin (see fyne.io/systray nativeLoop).
...
func runTray(cfg TrayConfig, icon []byte) error {
	if platformTrayRequiresMainThread() {
		// Caller must invoke StartTray from the main goroutine after LockOSThread in init.
		runtime.LockOSThread()
		runSystrayBlocking(cfg, icon)
		return nil
	}
...
func runSystrayBlocking(cfg TrayConfig, icon []byte) {
