...
| **`getnetworkinfo`** | **`localaddresses`** lists P2P listen bind + per-connection local IPs (not Core `mapLocalHost` discovery). **`networks[]`** includes onion as unreachable (no Tor integration). |
