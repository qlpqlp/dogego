# Raccoon-G-44 â€” in-tree port

> âš  **Experimental.** This directory is the in-tree C port of the
...
## Credits (Dogecoin Foundation)

In-tree Raccoon-G-44 port for libdogecoin / Core green: **Ed Tubbs**
([github.com/edtubbs](https://github.com/edtubbs),
[@EdTubbs](https://x.com/EdTubbs)). Vendored into DogeGo under this tree;
see `docs/CREDITS.md` in the DogeGo repo.
...
| `RACCOONG_NI`  | 560750930183101      | `RACC_NI` (= nâ»Â¹ mod q) |
| `RACCOONG_LOG_Q` | 50                 | âŒˆlogâ‚‚ qâŒ‰                |
...
| `RACCOONG_PK_BYTES`  | 16144          | 16-byte A_seed + 9Â·256Â·7 |
| `RACCOONG_SK_BYTES`  | 32272          | pk || 9Â·256Â·7 (s mod q) |
| `RACCOONG_SIG_BYTES` | 20768          | 32B c_hash + 9Â·256Â·7 z + 9Â·256Â·2 h |
...
3. Gaussian sampler is byte-exact against the recorded SHAKE256 driver
   prefix at ÏƒÂ² = 2â´â° and Ïƒâ‚œÂ² = 2Â¹â´ (MPFR @256-bit; mpmath@256 cross-check
...
5. `xof_sample_q` (uniform ð”½_q rejection sampler over SHAKE128) is
   byte-exact across four `(A_seed, i, j)` / `(key, i, j, k)` cells.
6. `expand_a` (9Ã-9 ExpandA), `vec_ntt` / `vec_intt` / `vec_add` /
...
7. `keygen_t_unrounded` (A_seed, sample of `s_i` / `e1_i`, and the full
   9Â·256 u64 unrounded `t = vec_intt(AÂ·s) + e1`) is byte-exact.
8. `thrc_keygen_from_seed` (HKDF-SHA256 â†’ NIST_KAT_DRBG â†’ unrounded keygen
   â†’ 7-byte LE serialization, pk=16144 B, sk=32272 B) is byte-exact
   against upstream `generate_keypair_from_seed`.
9. HD derive (`thrc_hd_derive_priv` / `_pub`) â€” HMAC-SHA512 tweak seed,
   coefficient-wise additive child keypair sharing the parent `A_seed` â€”
   is byte-exact against upstream `generate_tweak_keypair_from_seed`
   + `add_public_keys` + `add_signing_keys` for both hardened and
   non-hardened paths.
10. Signature wire format (20768 B: 32B `c_hash` || 9Â·256Â·7B z mod q ||
    9Â·256Â·2B h mod q_w, h centered to [-q_w/2, q_w/2) on read) is
...
12. `thrc_sign` / `thrc_verify` (Algorithm 2 / 3 with unrounded `tÌ‚`,
...
|---------------|----------------------------------------------------|-------------|
| `polyr.{c,h}` | `Z_q[X]/(X^n+1)` polynomial arithmetic             | Session 3 âœ“ |
| `ntt.{c,h}`   | Forward / inverse NTT, pointwise multiply          | Session 4 âœ“ |
| `gaussian.{c,h}` | MPFR-backed rounded Gaussian sampler            | Session 5 âœ“ |
| `shake256.{c,h}` | FIPS 202 SHAKE256 + SHAKE128 (Keccak-f[1600])   | Session 6 âœ“ / 7a âœ“ |
| `keygen_kdf.{c,h}` | HKDF-SHA256 + NIST_KAT_DRBG (AES-256-CTR)     | Session 7d âœ“ |
| `thrc.{c,h}` `xof_sample_q` | Uniform Z_q rejection sampler (SHAKE128) | Session 7a âœ“ |
| `thrc.{c,h}` `expand_a` + matvec | 9Ã-9 ExpandA + `vec_ntt`/`mul_mat_vec_ntt` | Session 7b âœ“ |
| `thrc.{c,h}` `keygen_t_unrounded` | A_seed + s/e1 + (AÂ·s)+e1 (unrounded t) | Session 7c âœ“ |
| `thrc.{c,h}` `thrc_keygen_from_seed` | Full seed â†’ 7-byte LE pk/sk pipeline | Session 7d âœ“ |
| `thrc.{c,h}` `thrc_hd_derive_*`      | HMAC-SHA512 BIP32-style HD derive    | Session 7e âœ“ |
| `thrc.{c,h}` signature serializer    | 20768-byte canonical wire format     | Session 7f.1 âœ“ |
| `thrc.{c,h}` `hash_vec` / `chal_poly` | hash_vec / challenge poly oracles   | Session 7f.2 / 7f.3 âœ“ |
| `thrc.{c,h}` BUFF `mu`               | `SHAKE256(SHAKE256(pk)||msg)` BUFF mu | Session 7f.4 âœ“ |
| `thrc.{c,h}` `thrc_sign` / `thrc_verify` | Algorithm 2 / 3 glue + KATs       | Session 7f.5 âœ“ |
| `raccoong.{c,h}` | Public-shape glue called by `src/pqc_raccoon.c` | Wired âœ“ |
...
- `contrib/raccoon_g/gen_polyr_vectors.py` â€” generator (polyr.c)
- `contrib/raccoon_g/gen_ntt_vectors.py`   â€” generator (ntt.c)
- `contrib/raccoon_g/gen_gaussian_vectors.py` â€” generator (gaussian.c)
- `contrib/raccoon_g/gen_xof_sample_q_vectors.py` â€” generator (xof_sample_q)
- `contrib/raccoon_g/gen_matvec_vectors.py` â€” generator (ExpandA + mul_mat_vec)
- `contrib/raccoon_g/gen_keygen_t_vectors.py` â€” generator (unrounded keygen)
- `contrib/raccoon_g/gen_keypair_vectors.py`  â€” generator (seed â†’ pk/sk)
- `contrib/raccoon_g/gen_hd_derive_vectors.py` â€” generator (HD derive)
- `contrib/raccoon_g/gen_signature_serialize_vectors.py` â€” generator
  (signature wire format)
- `contrib/raccoon_g/gen_hash_vec_vectors.py` â€” generator (hash_vec)
- `contrib/raccoon_g/gen_chal_poly_vectors.py` â€” generator (chal_poly)
- `contrib/raccoon_g/gen_buff_mu_vectors.py`  â€” generator (BUFF mu)
- `contrib/raccoon_g/gen_sign_vectors.py`     â€” generator (sign/verify KAT)
- `test/data/raccoong_*_vectors.h`            â€” generated C-array fixtures
- `test/data/raccoong_kat.json`               â€” top-level sign/verify
