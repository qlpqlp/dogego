...
- [x] **MVP:** **reboot testnet solo founder** - continue when peer probe fails (real scrypt PoW; Digishield min-diff); local genesis block; background **`mine`** / `generatetoaddress`; multi-peer **listen/outbound** while solo; periodic attach to **addnode** peers (`node/solo.go`, `node/solo_p2p.go`)
- [x] **MVP:** **solo background miner mempool inclusion** - `RunSoloMiner` / `generatetoaddress` legacy blocks include mempool txs (GBT-style `SelectBlockTemplateTxs`; coinbase value = subsidy + fees; `rpc/generate.go`, `generate_mempool_test.go`)
