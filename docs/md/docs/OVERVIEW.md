...
- **P2P**: single outbound peer, headers-first sync, optional raw block fetch windows, JSON-RPC subset (`rpc/dispatch.go` - see `SupportedMethods`; includes **`gettxout`**, **`gettxoutproof`** / **`verifytxoutproof`**, **`decodescript`**, **`createrawtransaction`**, with raw-block spend scan for `gettxout` - not Core UTXO DB).
